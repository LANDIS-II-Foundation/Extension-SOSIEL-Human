...
namespace Factory
{
    ...
    {
        private static Configuration<T> ReadModelConfig<T>() where T : class
        {
            Type type = typeof(T);
            ...
            return ConfigurationParser.ParseModelConfiguration<T>(File.ReadAllText(configFilePath));
        }

        private static Configuration<T> JoinConfigs<T>(AlgorithmConfiguration algorithm, Configuration<T> model) where T : class
        {
            model.AlgorithmConfiguration = algorithm;
