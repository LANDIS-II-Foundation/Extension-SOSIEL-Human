...
namespace Landis.Extension.Sosiel.Helpers
{
    using Entities;
    using Models;
    ...
    public static class StatisticHelper
    {
        ...
        public static Landis.Extension.SosielValuesOutput CreateLandis.Extension.SosielValuesRecord(IAgent[] activeAgents, int iteration, params string[] variableNames)
        {
            ...
            return new Landis.Extension.SosielValuesOutput { Iteration = iteration, Values = valueItems };
        }

        public static SubtypeProportionOutput CreateLandis.Extension.SosielPoolSubtypeProportionRecord(IAgent[] activeAgents, int iteration, int subtype)
        {
            ...
            if (suitableAgents.Length > 0)
            {
                proportion = suitableAgents.Average(a =>
                {
                    ...
                    {
                        return (double)a[VariablesUsedInCode.Landis.Extension.SosielPoolSubtupeProportion];
                    }
                    else
                    {
                        return a.ConnectedAgents.Count(a2 => (int)a2[VariablesUsedInCode.AgentSubtype] == subtype) / (double)a[VariablesUsedInCode.Landis.Extension.SosielPoolSize];
                    }
                });
            }
        ...
        public static Landis.Extension.SosielPoolSubtypeFrequencyOutput CreateLandis.Extension.SosielPoolFrequencyRecord(IAgent[] activeAgents, int iteration, int subtype)
        {
            Landis.Extension.SosielPoolSubtypeFrequencyOutput sf = new Landis.Extension.SosielPoolSubtypeFrequencyOutput { Iteration = iteration };
        ...
        public static Landis.Extension.SosielPoolSubtypeFrequencyWithDisturbanceOutput CreateLandis.Extension.SosielPoolFrequencyWithDisturbanceRecord(IAgent[] activeAgents, int iteration, int subtype, double disturbance)
        {
            Landis.Extension.SosielPoolSubtypeFrequencyWithDisturbanceOutput record = (Landis.Extension.SosielPoolSubtypeFrequencyWithDisturbanceOutput)CreateLandis.Extension.SosielPoolFrequencyRecord(activeAgents, iteration, subtype);
